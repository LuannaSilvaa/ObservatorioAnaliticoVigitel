...
const appSource = fs.readFileSync(path.join(root, 'assets', 'js', 'core', 'SistemaAnaliticoDoObservatorio.js'), 'utf8');
const dataSource = fs.readFileSync(path.join(root, 'assets', 'js', 'dados', 'BaseAnaliticaDoVigitel.js'), 'utf8');
...
const target = {
  innerHTML: '', attrs: {},
  /** Registra atributos aplicados ao SVG simulado durante a renderização. */
  setAttribute(name, value) { this.attrs[name] = String(value); },
  /** Representa consultas internas do SVG que não precisam retornar elementos neste teste. */
...
const context = {
  ...
  UF_NAMES:{},
  /** Mantém o agrupamento recebido porque o teste não aplica transformação de categorias. */
  effectiveGroup:g=>g,
  /** Converte a categoria em texto estável para a simulação da legenda. */
  categoryToken:l=>String(l),
  /** Mantém todas as categorias visíveis durante a conferência dos gráficos. */
  isHiddenCategory:()=>false,
  /** Considera todas as categorias compatíveis com a busca simulada. */
  matchesLegendSearch:()=>true,
  /** Devolve o SVG simulado ou um campo de configuração conforme o seletor consultado. */
  $:selector=>selector==='#chart'?target:field(selector.replace('#','')),
  /** Representa consultas múltiplas que não precisam de elementos neste teste. */
  $$:()=>[],
  target,
...
/**
 * Agrega valores por UF para validar o mapa com as mesmas linhas reais usadas pelo painel.
 */
function stateSeries(indicatorIndex) {
  const totals = new Map();
  for (const row of DATA.rows) {
    if (row[C.ind] !== indicatorIndex || row[C.pop] !== 0) continue;
    const uf = DATA.dims.ufs[row[C.uf]];
    const current = totals.get(uf) || {num:0, den:0, n:0};
    current.num += Number(row[C.num]) || 0;
    current.den += Number(row[C.den]) || 0;
    current.n += Number(row[C.n]) || 0;
...
const chartTypes=['line','area','bar','horizontal','ranking','lollipop','pareto','pie','donut','radar','kpi','gauge','map','treemap'];
const failures=[];
let rendered=0;
for (let indicatorIndex=0; indicatorIndex<DATA.indicators.length; indicatorIndex++) {
  ...
  for (const type of chartTypes) {
    ...
    try {
      const renderSeries = type === 'map' ? stateSeries(indicatorIndex) : series;
      const renderGroup = type === 'map' ? 'UF' : 'Ano';
      context.drawChart(renderSeries,renderGroup,target,type,context.S.graphMeta);
      /** Exige no mapa os 27 contornos estaduais vetoriais, além da identificação comum aos gráficos. */
      const mapOk=type!=='map' || (target.innerHTML.match(/class="map-state-shape"/g)||[]).length===27;
      const ok=target.innerHTML.includes('<svg') && target.attrs['data-rendered-chart']===type && target.attrs['data-chart-type']===type && mapOk;
...
const report=[
  `TESTE DE TODOS OS INDICADORES EM TODOS OS GRÁFICOS — ${DATA.meta.baseVersion || 'BASE ATUAL'}`,
  '='.repeat(66),
...
fs.writeFileSync(path.join(root, 'relatorios', 'RelatorioDosIndicadoresEGraficos.txt'),report,'utf8');
console.log(report);
