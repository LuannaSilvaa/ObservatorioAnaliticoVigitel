...
#'
#' @param x Vetor numérico ou textual vindo dos microdados.
#' @return Vetor numérico com valores inválidos convertidos em ausentes.
#' @details A conversão reconhece a vírgula como separador decimal e remove o ponto usado como separador de milhar antes de aplicar `as.numeric`.
numero <- function(x) {
...
#'
#' @param base Tabela da edição em processamento.
#' @param nome Nome original da variável no arquivo do Vigitel.
#' @return Vetor numérico da coluna ou vetor ausente com o mesmo número de linhas da base.
#' @details O vetor ausente permite que as regras testem a disponibilidade da variável sem alterar o número de registros.
...
#'
#' @param base Tabela da edição em processamento.
#' @param variavel Nome da variável consultada.
#' @param validos Códigos que pertencem ao denominador.
#' @param positivos Códigos que pertencem ao numerador.
#' @return Lista com as máscaras lógicas elegivel e evento.
#' @details Respostas fora de `validos`, inclusive códigos de recusa ou desconhecimento, não entram no denominador.
...
#'
#' @param base Tabela da edição em processamento.
#' @param variavel Nome da variável binária.
#' @param positivo Código que representa a ocorrência do indicador.
#' @return Lista com as máscaras lógicas elegivel e evento.
#' @details Apenas zero e um são aceitos no denominador; qualquer outro código permanece fora do cálculo.
...
#'
#' @param base Tabela com peso imputado em `q9_i` e altura imputada em `q11_i`.
#' @param tipo Categoria solicitada: `adequado` ou `baixo`.
#' @return Lista com elegibilidade e ocorrência da categoria de IMC solicitada.
#' @details A função limita o IMC ao intervalo plausível adotado no projeto antes de classificar a categoria.
...
#'
#' @param base Tabela já harmonizada por `preparar_base`.
#' @param indicador_id Código estável do indicador no painel.
#' @return Lista com elegibilidade e evento, ou valor nulo quando a edição não possui as variáveis exigidas.
#' @details Cada condição corresponde à regra registrada no dicionário metodológico. Uma regra nova deve ser incluída também no gerador Python e nos testes.
calcular_evento <- function(base, indicador_id) {
  ...
  #'
  #' @param vars Nomes das variáveis exigidas pela regra.
  #' @return Valor lógico indicando se a regra pode ser calculada.
  #' @details A verificação evita transformar a ausência de uma pergunta em prevalência igual a zero.
...
#'
#' @param base Microdados de uma edição do Vigitel.
#' @param nome_arquivo Nome usado para identificar o ano e o tipo de população.
#' @return Base pronta para o cálculo ponderado, com registros inválidos removidos.
#' @details A população geral prioriza `pesorake2025`; o recorte de população negra usa `pesorake_cor` e exige `q69_cor`.
...
#'
#' @param base Tabela harmonizada com a coluna `peso_analise`.
#' @param indicador_id Código do indicador que será calculado.
#' @return Linha de resultado ponderado ou valor nulo quando não existem respostas elegíveis.
#' @details O resultado conserva contagem de entrevistas, casos e soma dos pesos ao quadrado para as verificações de precisão.
...
# resultado <- do.call(rbind, lapply(INDICADORES_DO_PAINEL, function(id) prevalencia_ponderada(base, id)))
