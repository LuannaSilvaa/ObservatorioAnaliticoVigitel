...
(function(){
  ...
  function getGlossaryEntry(indicator){
    ...
    return {
      ...
      caution:THEME_CAUTIONS[indicator.themeId] || 'Considere ponderação amostral, tamanho da amostra, mudanças metodológicas e intervalos de confiança ao comparar resultados processados.',
  ...
  function renderList(selectedId){
    ...
    if(window.matchMedia?.('(max-width: 760px)').matches){
      window.requestAnimationFrame(()=>{
        const selectedButton=list.querySelector('[data-glossary-id].active');
        if(selectedButton){
